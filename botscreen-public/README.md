...
**关于联网，三件容易误判的事**（依据：本仓库跟踪的
`3.在线AI语音识别语音合成SDK使用教程(Linux、ROS)/离线和在线SDK版本功能说明.txt`）：

1. **语音识别是在线服务。** 该文件写明离线版只做命令词识别（≤16 个汉字、须先构建语法），
   自由问答识别用的是在线版 AIUI 的语音听写。所以断网时的表现是「**进程仍然活着，但没有识别结果**」——
   不是进程崩了，重启进程解决不了，只能等网络恢复。
2. **出网的不止语音。** 语音转发节点本身只连回环（`server/voice_transfer_node_ros2.py` 的
   `API_URL`），但**回答引擎也可能出网**：`legacy` 未命中知识库时会请求 DeepSeek。
   「断网还能不能演示」取决于当次用哪个引擎、知识库命中多少，不能一概而论。
3. **语音合成这一环尚未核实。** 上面那份文件说在线版功能集包含语音听写**和语音合成**，
   但本链路自己写的是「页面 TTS 播报」，实际由谁播报（SDK 合成 vs 浏览器内置）没有结论。
